...
unsigned int array_from_file(int array[], unsigned int max_size, const char *filepath) {
    FILE *file = NULL;
    file = fopen(filepath, "r"); //el puntero esta al inicio del archivo
    if (file == NULL) {
    ...
    int res = 0;
    res = fscanf(file, " %u ", &size); //el puntero lee el primer numero del archivo y se mueve automaticamente
    if (res != 1) { //si logra leerlo, devuelve 1
    ...
    while (i < size) {
        res = fscanf(file," %d ", &(array[i])); //empieza a leer desde donde quedo el puntero, lee el sig numero del archivo y avanza
        if (res != 1) {
            fprintf(stderr, "Invalid array.\n");
            exit(EXIT_FAILURE);
        }
